...
outputs_folder_path = './sample_audio'

print(os.listdir('./sample_audio'))
# List all the patient record files (assuming the extension for audio files is .mp3)
patient_records = [f for f in os.listdir(recordings_folder_path) if f.endswith('.aac')]
...
st.audio(audio_file.read(), format='audio/aac')
...
summary_file_path = os.path.join(outputs_folder_path, selected_record.replace('.aac', '.txt'))
